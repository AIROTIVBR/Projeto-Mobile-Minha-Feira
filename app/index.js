...
const CancelButton = styled.TouchableOpacity`
  border: 2px solid #fff; /* Borda da cor 192A1A */
      ...
      <FlatList
        ...
        renderItem={({ item }) =>
          item.isNewListButton ? (
            <ThinNewListButton
              onPress={() => navigation.navigate("CreateFromCollection")}
            >
              <PlusIcon name="plus" />
...
function BuyScreen() {
  ...
  if (lists.length === 0) {
    return Alert.alert("", "Você ainda não criou listas !", [
...
function CreateScreen({ navigation }) {
  ...
  const handleConclude = () => {
    if (listName.trim() == "") {
      ...
      Alert.alert("", "É necessário adicionar itens á lista.");
    } else {
...
function EditListScreen({ route, navigation }) {
  const { list } = route.params;
  const { addList, deleteList, updateList, updateItem } =
  ...
  const handleEditItem = (index, newName, newQuantity) => {
    ...
    if (updatedItem.name.length == 0) {
      Alert.alert("Nome do item não pode ser vazio!");
    } else {
      setItems((prevItems) => {
      ...
      <Body style={{ marginTop: -10 }}>
        ...
        >
          ...
          <TouchableOpacity onPress={handleDeleteList}>
            <Feather name="trash" color="#79fc86" size={36} />
          </TouchableOpacity>
        </View>
          ...
          renderItem={({ item, index }) => (
            <ItemRow>
              <Counter>
                ...
                <CounterButton
                  onPress={() =>
                    handleEditItem(index, item.name, item.quantity + 1)
                  }
                >
                  <CounterText>+</CounterText>
                </CounterButton>
              ...
              <ItemInput
                value={item.name}
                onChangeText={(text) =>
                  handleEditItem(index, text, item.quantity)
                }
                placeholder="Nome do item"
              ...
              <ExcluirButton
                onPress={() => {
                  Alert.alert(
                    "",
                    "Tem certeza que deseja apagar esse item da sua lista?",
                    [
              ...
              >
                <Feather name="x-circle" size={26} color="#F00" />
              </ExcluirButton>
            </ItemRow>
          )}
        />
...
export default function App() {
  ...
  return (
    <ListContext.Provider
      value={{ lists, addList, deleteList, updateItem, updateList }}
    >
      <Tab.Navigator
        tabBar={(props) => (
          <StyledTabBar {...props}>
            {props.state.routes.map((route, index) => {
              ...
              return (
                <TabButton key={index} onPress={onPress}>
                  <TabIcon
                    name={route.name === "Home" ? "home" : "archive"}
                    focused={isFocused}
                    size={24}
                  />
                  <Text style={{ color: isFocused ? "green" : "white" }}>
                    {route.name}
                  </Text>
                </TabButton>
              );
